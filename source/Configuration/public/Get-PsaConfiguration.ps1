Function Get-PsaConfiguration {
    ...
    param(
        ...
        # is set, it is passed to `Get-PsaConfigurationPath`
        [Parameter(
            ValueFromPipeline
        )]
        [string]$Path,
    ...
    process {
        ...
        if ($ConfigPath -is [System.IO.FileInfo]) {
            ...
            $configuration = Import-Psd $ConfigPath
            ...
            Get-ChildItem -Path $ConfigPath -Filter '*.config.psd1' | ForEach-Object {
                ...
                switch ($conf_parts.Count) {
                    1 {
                        ...
                        $configuration[$parent] = Import-Psd $file.FullName
                        continue
                    }
                    2 {
                        $parent = $conf_parts[0]
                        ...
                        $configuration[$parent][$child] = Import-Psd $file.FullName
                        continue
                    }
                    3 {
                        $parent = $conf_parts[0]
                        ...
                        $configuration[$parent][$child][$sub] = Import-Psd $file.FullName
                        continue
